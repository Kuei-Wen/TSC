...
if __name__ == '__main__':
    ...
    """
使用說明
    將您的影片檔（例如 my_video.mp4）和 Python 腳本 create_subtitles.py 放在同一個資料夾。
將您下載並解壓縮的 Vosk 模型資料夾（例如 vosk-model-cn-0.22）也放在同一個資料夾。
你的資料夾看起來應該像這樣：
/your_project_folder
├── create_subtitles.py
├── my_video.mp4
...
打開終端機或命令提示字元，切換到該資料夾目錄。
執行以下命令：
python create_subtitles.py my_video.mp4
腳本會開始運行，並顯示每一步的進度。
查看結果：

完成後，您會在資料夾中看到兩個新檔案：
my_video.srt: 純文字的字幕檔，您可以手動編輯校對。
my_video_subtitled.mp4: 包含字幕的新影片檔。
播放 my_video_subtitled.mp4 時，您可以在播放器（如 VLC, PotPlayer）的字幕選單中選擇開啟或關閉這個字幕軌道。
